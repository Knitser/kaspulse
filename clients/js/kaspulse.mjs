/* kaspulse.mjs — a tiny zero-dependency VERIFYING client for the kaspulse oracle.
   Works in Node 18+ and the browser (native fetch + BigInt). No npm deps, no keys.

     import { Kaspulse } from './kaspulse.mjs';
     const k = new Kaspulse('http://localhost:8080');
     const feed = await k.feed('KAS/USD');
     const r = k.verifyFeed(feed);      // 3-of-5 BIP340 over blake2b-256, field-bound
     const px = k.checkedValue(feed);   // verified + fresh + not halted, or throws

   Honest scope: this verifies the committee's signatures, the binding of the
   signed message to the JSON fields, freshness and the safety flags. It does
   NOT re-fetch the exchanges and recompute the median — that is
   `cargo run --bin verify`.

   The crypto core self-tests at load (BIP340 official vector 0, a corrupted
   copy that must FAIL, and blake2b-256("abc")); on any failure every verify
   call throws 'verifier self-test failed' rather than risk a fake ✓.

   CLI:  node kaspulse.mjs verify KAS/USD [base]
...
// ── errors + HTTP client ────────────────────────────────────────────────────
export class NoSuchFeedError extends Error {
  constructor(pair) {
    super(`kaspulse: no such feed: ${pair}`);
...
export class Kaspulse {
  ...
  async #get(path) {
    ...
    if (!res.ok) throw new Error(`kaspulse: ${path} → HTTP ${res.status}`);
    return res.json();
  }
  ...
  async feed(pair) {
    const path = `/v1/feed/${String(pair).replace(/\//g, '-')}`;
    const res = await fetch(`${this.base}${path}`, { headers: { accept: 'application/json' } });
    if (res.status === 404) throw new NoSuchFeedError(pair);
    if (!res.ok) throw new Error(`kaspulse: ${path} → HTTP ${res.status}`);
    return res.json();
  }

  /** Verify a FeedObj locally — threshold of BIP340 signatures over
  ...
  checkedValue(feed, { maxAgeMs = 30000 } = {}) {
    const r = verifyFeedCore(feed);
    if (!r.ok) throw new Error(`kaspulse: refusing value: ${r.error || 'verification failed'}`);
    if (feed.halted) throw new Error('kaspulse: refusing value: feed halted (circuit breaker)');
    if (feed.peg_ok === false) throw new Error('kaspulse: refusing value: chain depegged (peg_ok=false)');
    const ageMs = Date.now() - Number(feed.signed_ts) * 1000;
    if (!Number.isFinite(ageMs) || ageMs > maxAgeMs) {
      throw new Error(`kaspulse: refusing value: signature is ${Math.round(ageMs)} ms old (max ${maxAgeMs} ms)`);
    }
    return Number(feed.mant) * Math.pow(10, Number(feed.expo));
  }
}

// standalone forms of the core, for the curious and the paranoid
export function verifyFeed(feed) { return verifyFeedCore(feed); }
export { blake2b256, bip340Verify, selfTest };
export default Kaspulse;

// ── CLI: node kaspulse.mjs verify KAS/USD [base] ────────────────────────────
if (typeof process !== 'undefined' && !!process.versions?.node && process.argv[1]
    && import.meta.url === (await import('node:url')).pathToFileURL(process.argv[1]).href) {
  const [cmd, pairArg, baseArg] = process.argv.slice(2);
  if (cmd !== 'verify' || !pairArg) {
    console.error('usage: node kaspulse.mjs verify KAS/USD [base]');
    process.exit(2);
  }
  const st = selfTest();
  if (!st.ok) {
    console.error(`✗ verifier self-test failed: ${st.detail}`);
    process.exit(1);
  }
  const base = baseArg || DEFAULT_BASE;
  const pair = pairArg.replace(/-/g, '/');
  try {
    const k = new Kaspulse(base);
    const feed = await k.feed(pair);
    const r = k.verifyFeed(feed);
    console.log(`${feed.pair}  ${base}  signed_round ${feed.signed_round ?? '?'}`);
    r.results.forEach((n, i) => console.log(`  node ${i}  ${n.ok ? '✓' : '✗'}  ${n.signer}`));
    console.log(`  bound=${r.bound} (signed message fields == JSON fields)`);
    if (r.ok) {
      const px = Number(feed.mant) * Math.pow(10, Number(feed.expo));
      console.log(`✓ VALID — ${r.valid}/${r.results.length} signatures verify (threshold ${r.threshold}), price = ${px}`);
      process.exit(0);
    }
    console.log(`✗ INVALID — ${r.error || 'verification failed'}`);
    process.exit(1);
  } catch (e) {
