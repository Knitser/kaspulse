...
- **`js/kaspulse.mjs`** — Node 18+ / browser, native fetch + BigInt.
  Default base `https://pulse.kascov.io`.
  `node js/kaspulse.mjs verify KAS/USD`
- **`py/kaspulse.py`** — Python 3.9+, stdlib only.
  Default base `https://pulse.kascov.io`.
  `python3 py/kaspulse.py verify KAS/USD`
...
Both clients expose `committee()` / `verifyWithCommittee` (JS) and
`committee()` / `verify_with_committee` (Python) for pinning against `/v1/committee`.
